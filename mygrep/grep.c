...
int main(int argc, char *argv[]) {
	...
	init_stack_source(); 		// to initialise all sorts of list
	addinsource(argc, argv);	 // files, patterns and directories if any, are added here
	addinstack(); 			// output which my program shows is get added here
	initz(&fl);			// files are initialised
	if(dc.flag_c == 1) {
		printcount(); 		// to print count if demanded 
		destroy_stack_source();// to destroy all sorts of list
		return 0;
	}
	printoutput();
	destroy_stack_source(); 
	return 0;
}
